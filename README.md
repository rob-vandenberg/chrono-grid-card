 ...
 <div align="center">
  ...
  <img src="art/header.svg" width="780" alt="Chrono Grid Card Banner">


  <img src="art/banner.png" width="800" alt="Chrono Grid Card Banner">
  ...
  <p align="center">
    <strong>A flexible multi-field text card for Home Assistant dashboards.<br>
            ...
            Grid, HTML, and live Jinja2 templates.</strong>
  </p>
...
**Chrono Grid Card** is a drop-in replacement and upgrade for Home Assistant's built-in Grid card. Instead of a fixed title and content structure, it renders any number of independently styled text fields stacked vertically inside a single card. Each field supports Grid, raw HTML, and live Jinja2 templates — so any configuration that works in the standard Grid card works here too, with zero migration effort.

The card matches the default appearance of HA's Grid card exactly out of the box, while adding a full visual editor with per-field typography, color, border, and spacing controls that the standard card simply does not offer.
...
### ✍️ Grid, HTML, and Jinja2 — All at Once
Each field's content is evaluated as a Jinja2 template (via Home Assistant's WebSocket connection) and then rendered through the same Grid engine used by HA's own Grid card. Raw HTML is fully supported alongside Grid in the same content field. Any content that works in HA's standard Grid card works here without modification.
...
### 🔄 Seamless Migration from HA's Grid Card
The card is designed as a drop-in replacement. Copy the content from any existing Grid card, paste it into a Chrono Grid Card field, and it renders identically. No syntax changes, no reformatting required.
...
4. Enter `https://github.com/rob-vandenberg/chrono-grid-card` and select **Lovelace** as the category.
5. Click **Add**. The repository will appear in the list.
6. Search for `Chrono Grid Card` and click **Download**.
7. Reload your browser.

### Manual Installation

1. Download `chrono-grid-card.js` from the [latest release](https://github.com/rob-vandenberg/chrono-grid-card/releases/latest).
2. Copy it to your Home Assistant `config/www/` folder.
3. In Home Assistant, go to **Settings → Dashboards → Resources**.
4. Click **Add Resource**.
5. Enter `/local/chrono-grid-card.js` as the URL and select **JavaScript Module**.
...
2. Find **Chrono Grid Card** and click the three-dot menu.
3. Select **Remove**.
4. Reload your browser.

### Manual
1. Delete `chrono-grid-card.js` from `config/www/`.
2. Remove the resource entry from **Settings → Dashboards → Resources**.

---

<img src="art/editor.png" width="800" alt="Chrono Grid Card Editor">
...
| `line_breaks` | boolean | `false` | When enabled, a single Enter key creates a new line in the rendered output. When disabled, two Enter keys are required (standard Grid paragraph behaviour). |
| `content` | string | `''` | Field content. Supports Grid, HTML, and Jinja2 templates. |
| `color` | string | HA theme | Text color. Empty = HA theme default (`--primary-text-color`). |
| `font_size` | number | `1.0` | Font size in em. Grid headings (`#`, `##`, etc.) scale proportionally relative to this value. |
